...
class RecordsController {
    ...
    public async newRecord(req: RequestWithUser, res: Response, next: NextFunction) {
        try {
            ...
            // if ((!(newRecordInfo.number! >= 0) && !newRecordInfo.color) || (newRecordInfo.number! >= 0 && newRecordInfo.color)) {
            //     throw new HttpException(statusCode.BAD_REQUEST, "Invalid number or color!");
            // }
